...
class LLMFiLMGenerator(nn.Module):
    ...
    def build_prompt(self, instruction: str, episode_num: int, total_episodes: int, prompt_id: int = 1) -> str:
        ...
        if prompt_id == 1:
            ...
            prompt = f"""You are a global optimizer tasked with finding the global minimum of f(params) over {self.rank} continuous variables in [-6, 6].

            ### # Information:
            - **Approximate global optimum**: {self.optimum_reward:.1f}
            - **Exploration step size**: {self.step_size}
            - **Previously tested points and results**:

            {history_text}

            Use the history to adaptively balance exploration and exploitation as iterations progress.

            ### # Output Format:
            You must provide your response in **exactly two blocks** in this order:

            1. **<think>**
            Perform a detailed analysis of the current state:
            - **State Summary**: Identify the Global Best step from the history buffer and its proximity to {self.optimum_reward:.1f}.
            - **Trend Analysis**: Compare the most recent trials. Calculate the explicit Δf = f(t) - f(t-1) to determine the current trajectory.
            - **Step Evaluation**: Analyze how the previous change in parameters affected the objective value.
            - **Strategy & Pacing**: State whether you are **Exploring** (using step ≈ {self.step_size}) or **Exploiting** (smaller step). Justify this based on the iteration index {episode_num} to pace the search (starting broad, narrowing later).
            </think>
            ...
            ### # Rules:
            - **Do not reuse** any params from the history buffer.
            - **Precision**: Ensure each value has 2 decimal precision within [-6.00, 6.00].
            - **Formatting**: Provide strictly the two blocks above. Do not provide any additional text or lines outside the XML tags.

            **Now at iteration {episode_num} of {total_episodes}.** Provide your response."""
    ...
    def generate_film_params(self, instruction: str, episode_num: int, total_episodes: int, device: torch.device = None, prompt_id: int = 1) -> Tuple[torch.Tensor, torch.Tensor, str]:
        ...
        if episode_num == 0:
            ...
            explanation = "Initial episode: using base parameters (gamma=1, beta=0)"
