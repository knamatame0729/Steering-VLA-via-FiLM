...
# FIXED_GOAL_POS = np.array([0.0, 0.85, 0.2], dtype=np.float64)
...
#         self._saved_init_state = None  # Will be set on first reset with seed
#         obs, _ = self.env.reset(seed=seed)

#         # Force a deterministic fixed goal position for all episodes.
#         self._set_fixed_goal(self.env.unwrapped)
#         obs = self.env.unwrapped._get_obs()
        
#         # Save the initial state immediately so all future resets use the same state
...
#     def _set_fixed_goal(self, unwrapped):
#         """Apply the fixed goal position to env internals and goal site marker."""
#         goal_pos = FIXED_GOAL_POS.copy()
#         unwrapped.goal = goal_pos
#         if hasattr(unwrapped, '_target_pos'):
#             unwrapped._target_pos = goal_pos.copy()

#         goal_site_id = mujoco.mj_name2id(unwrapped.model, mujoco.mjtObj.mjOBJ_SITE, 'goal')
#         unwrapped.model.site_pos[goal_site_id] = goal_pos.copy()
#         if hasattr(unwrapped.model, 'site'):
...
#         On first call with a seed, saves the initial state.
#         All subsequent resets will restore the exact same initial state to ensure
#         reproducibility across episodes (same robot pose, object position, goal).
#         """
#         # First, always call the underlying reset
#         obs, info = self.env.reset(seed=seed)
#         unwrapped = self.env.unwrapped

#         # Enforce a fixed goal on every reset.
#         self._set_fixed_goal(unwrapped)
        
#         # If we have a saved initial state, restore it (takes priority)
#         if self._saved_init_state is not None:
...
#             # Restore the goal site position (controls visual goal marker)
...
#             # Forward the simulation to apply the restored state
#             mujoco.mj_forward(unwrapped.model, unwrapped.data)
            
#             # Get updated observation after restoring state
#             obs = unwrapped._get_obs()
#         elif seed is not None:
#             # First reset with seed - save the initial state for ALL future resets
#             # Get the goal site position for visual marker
#             goal_site_id = mujoco.mj_name2id(unwrapped.model, mujoco.mjtObj.mjOBJ_SITE, 'goal')
...
#             obj_pos = np.array([
#                 self.ep_rng.uniform(-0.25, 0.25),   # x
#                 self.ep_rng.uniform(0.55, 0.7),    # y
#                 0.02                           # z
#             ])
