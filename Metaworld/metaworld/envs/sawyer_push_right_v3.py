...
class SawyerPushRightEnvV3(SawyerXYZEnv):
    """SawyerPushRightEnv

    Push the puck to the right side of the table.
    Based on SawyerPushEnvV3 with modified goal positioning.
    """
    ...
    def __init__(
        ...
        goal_low = (0.45, 0.55, 0.01)
        goal_high = (0.55, 0.65, 0.02)
        ...
        self.goal = np.array([0.5, 0.6, 0.02])
    ...
    def model_name(self) -> str:
        return full_V3_path_for("sawyer_xyz/sawyer_push_right_v3.xml")
    ...
    def reset_model(self) -> npt.NDArray[np.float64]:
        ...
        while np.linalg.norm(goal_pos[:2] - self._target_pos[:2]) < 0.15 or self._target_pos[1] <= goal_pos[1]:
            goal_pos = self._get_state_rand_vec()
            self._target_pos = goal_pos[3:]
